...
- **Translate the app-store search strings.** `home_search_play_store` and `app_store_search_unavailable` ship English-first with `tools:ignore="MissingTranslation"` + `<!-- TODO: translate -->` (the wording is now approved). Fan them out to the `values-*/strings.xml` locales and drop the deferral markers.
- **Empty state could show a loading placeholder while search evidence settles.** The no-match store action is now gated on `isFreshAppLoadComplete && !isLoadingSearchContent`, so it never offers a search while the app inventory or a content index is still loading. The "No matches" *message* itself can still flash for that brief window before the evidence settles; a nicer touch would be a loading placeholder (spinner / "Searching…") in that state instead of the no-match copy. Cosmetic only — the misleading one-tap exit is already closed. (Codex on PR #739.)
- **Accepted edges — no-match store action while the installed set itself is incomplete.** Scoped to the *empty state's* action: the in-list store-search row ranks under whatever matched and claims nothing about absence, so none of this gates it. `storeSearchReady` also requires `!queryMatchesInstalledApp` — an authoritative match against the *full* installed set (hidden and quiet-mode apps included), so a query that matches an installed-but-hidden app no longer offers a store search. What's left are the windows where that full set is itself momentarily or partially unavailable, so the match can't see the entry: all accepted rather than fixed (maintainer call on PR #739) — each is a recoverable Play Store open, brief, and a symptom of a list that is already transiently stale for every surface, not just this action; closing them means more readiness-state plumbing than a no-match button warrants. The fixes, if ever revisited:
  - **Degraded enumeration.** `isFreshAppLoadComplete` is set even when a profile read fails, so an app in the *unread* profile reads as a no-match. Fix: gate on the authoritative VM-private `isAppInventoryComplete` latch (LauncherViewModel ~731/~1330), exposed in `LauncherUiState`.
  - **App-list reloads.** `scheduleReload`/`runReload` (install / package-change / work-profile-availability) re-enumerate without resetting `isFreshAppLoadComplete`, so a just-changed app reads as a no-match in that sub-second window. Fix: track reload-in-progress in the gate.
  - **Content-read failure.** `loadContactIndex` / `loadSearchEventIndex` catch a provider `RuntimeException`, return empty, and the completion still clears `isLoadingSearchContent` — marking a failed read "settled." Fix: represent per-source load success separately from a completed read.
- **Preferred app store setting.** The app-store search (shipped — the apps section's lowest-ranked row, and the "No matches" empty state's action) defaults to Google Play. Add a Settings "preferred app store" so a device without Play — or a user on F-Droid / Aurora / a vendor store — can point it elsewhere; the intent builder in `LauncherIntents.kt` (`playStoreSearchIntent` / `playStoreSearchWebIntent`) is where the store URL would branch.
- **Web search action for a no-match query.** In the same "No matches" empty state as the app-store action (placement decided), add a globe / "world" action that runs the typed query as a web search. Open question still to settle: which engine — the system default browser's own search via `Intent.ACTION_WEB_SEARCH`, a fixed `https://www.google.com/search?q=` view, or a Settings-chosen engine.
