...
- **Consider an in-app banner (or similar cue) that detects when the hosted
  privacy policy has changed since the user last saw it, and surfaces that
  in Settings.** Play "What's new" never reaches a user who's already
  installed the app until they happen to open the store listing, so a
  real disclosure change ships with effectively no visibility today.
  Real cost before building it: checked commit history across the sibling
  apps and PRIVACY.md churn is bursty — frequent during a feature's initial
  buildout, then roughly monthly or less once the app stabilizes — so a
  change-detection banner would fire noisiest exactly when it matters least
  and rarely during the period it would matter most. Worth a second look if
  this app (or a sibling) reaches a stage where policy changes carry real
  stakes; not worth building speculatively now.
