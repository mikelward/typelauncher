...
- **Translate the no-match app-store strings.** `home_search_play_store` and `app_store_search_unavailable` ship English-first with `tools:ignore="MissingTranslation"` + `<!-- TODO: translate -->` (the wording is now approved). Fan them out to the `values-*/strings.xml` locales and drop the deferral markers.
