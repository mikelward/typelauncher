...
- Docked apps are excluded from the main app list (both typed and empty-query views) while the dock is enabled, since they already render in the dock row and listing them in both places would duplicate them and let a docked app outrank a non-docked match the user actually meant to type. The dock itself still re-filters by the same query, so a docked app that matches stays visible in its dock slot. If a typed query matches only a docked app — leaving the main list empty — pressing Enter / search falls back to launching the first matching dock entry so the keyboard launch flow still works for docked-only matches.
