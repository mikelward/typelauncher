...
- Typing filters installed apps with a case-insensitive anchored fuzzy match against the app label. The first query character must equal either the first letter of the label or one of the label's uppercase letters. From that anchor, the remaining query characters must appear in order anywhere in the rest of the label (subsequence) — non-capital letters in the label may be skipped. For example, `s` matches `Surfshark` (anchors at start) and `mySA` (anchors at the capital S) but not `1password` or `Ross` (no anchor letter equals `s`); `boa` matches `BofA` (anchors at B, then `o` and `A` appear in order); `gm` matches `Google Mail` (anchors at G, then capital M is a later subsequence character). `mail` does not match `Gmail` today because `m` is neither the first letter nor a capital — see TODO for the planned ranked relaxation.
...
- Time text is preprocessed so that whitespace inside each time half is non-breaking (U+00A0) and the separator between the start/end times is a regular-space-flanked en-dash (U+2013). When the row is wide enough the time stays on a single line; when the row is too narrow to hold the full range the text engine wraps at the dash (e.g. `12:00 PM` / `– 1:00 PM`) instead of splitting mid-time. The time column uses a 72dp minimum width but is allowed to grow to whatever a single-line range needs.
