...
- The app uses edge-to-edge layout and applies status bar, navigation bar, and IME insets through the top-level `Scaffold`. IME insets use `WindowInsets.imeAnimationTarget` rather than `WindowInsets.ime` so the content reserves the keyboard's final space immediately when the IME is requested, keeping tap targets stable on launch instead of sliding up alongside the keyboard animation.
