...
- The first agenda load is deferred until Home reports it is ready: the app list has finished loading (`isLoadingApps == false`) and the soft keyboard is visible (`WindowInsets.isImeVisible`), or 1500 ms have elapsed since the apps loaded — whichever happens first. The fallback timeout exists so hardware keyboards, IME-disabled environments, and Robolectric tests don't block the agenda load forever. The signal is one-shot per process: `LauncherViewModel.onHomeReady` is idempotent. This keeps the cold-start `Dispatchers.IO` load and main thread focused on the app list and the keyboard show, which is the launcher's primary interaction.
