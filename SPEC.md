...
- The active first filtered app is highlighted with the Material selected-row container color so the Enter/search launch target is visible.
...
- The add-widget page groups available home-screen widget providers under per-app headers and shows generated widget previews when providers expose them, falling back to static preview images or a placeholder when generated previews are unavailable or fail to inflate.
