...
11. `MainActivity` starts lifecycle collectors for keyboard-auto-show, theme mode, and home-ready, then calls `setContent` on the UI thread. The top-level `Scaffold` applies status/navigation bar insets only; Compose reserves a single stable keyboard-height slot from `WindowInsets.imeAnimationTarget` instead of following every animated `WindowInsets.ime` tick or double-applying `adjustResize`. The last non-zero target is persisted in `dock_settings` so warm starts and carousel returns can reserve keyboard-height geometry before the next IME target arrives.
