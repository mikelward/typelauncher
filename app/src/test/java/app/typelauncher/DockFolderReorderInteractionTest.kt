...
class DockFolderReorderInteractionTest {
    ...
    @Test
    fun draggingAMemberReordersInRowLayout() {
        // Reorder is arrangement-agnostic: it settles against the rendered member
        // cell centers, not the slot math, so the Row folder open layout drags and
        // reorders exactly like the default. This asserts that directly — the same
        // two-neighbor drag, but with FolderOpenLayout.Row selected (members packed
        // contiguously from the top-left, close tile last).
        val apps = listOf("App01", "App02", "App03", "App04").map { member(it) }
        val byId = apps.associateBy { it.id }
        var order by mutableStateOf(apps.map { it.id })
        composeRule.setContent {
        ...
        composeRule.waitForIdle()
        assertEquals("first step should move App01 one slot in Row layout", 1, order.indexOf(app01))
        ...
        val finalIndex = order.indexOf(app01)
        assertTrue(
            "App01 must reorder across two neighbors in the Row layout " +
                "(ended at index $finalIndex in $order)",
            finalIndex >= 2,
        )
    }
}
